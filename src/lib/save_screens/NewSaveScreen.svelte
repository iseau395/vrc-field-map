...
<script lang="ts">
    ...
    import { GameType } from "../map/games/game";
    import { reset } from "../map/field";
    import Textbox from "../components/Textbox.svelte";
    import { current_save_id, get_save_state, load_save_state } from "../map/saving";
    ...
    function create_save() {
        if (!selected_game_type || !save_name) return;
        $new_save_screen_open = false;

        switch (selected_game_type) {
            case "Over Under": {
                $game_type = GameType.OverUnder;
            } break;
            case "Spin Up": {
                $game_type = GameType.SpinUp;
            } break;
        }

        $is_skills = skills;
        $current_save_id = save_name;
